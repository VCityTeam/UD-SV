...
This chapter describes two different methods for installing a [3DCity Data Base](https://www.3dcitydb.org/3dcitydb/) server :
 - a quick an easy [3DCityDB **docker install**](#1a-installing-a-3dcitydbpostgis-server-the-docker-deployment-case)
 - a customizable [native package (apt) level installation](#1b-installing-a-3dcitydbpostgis-server-the-linux-server-configuration-case)
 ...
 * Troubleshooting:
   ...
   then some other service is already using port 5432. You now have three cases concerning the 5432 port:
     ...
     1. it was pre-empted by another service/container that you _do_ need (for example the [default API_Enhanced_City configuration](https://github.com/MEPP-team/UD-Serv/blob/master/API_Enhanced_City/INSTALL.md#install-using-docker), that you might have installed for your UD-Viz demo, uses that port)<br>
